...
rules:
  - list of adjectives, nouns, verbs, adverbs
  - plug appropriate word into template
  
algorithm:
  make template with word types for insertion
  replace each word type with replacer function
  
  replacer
    - generate random index
    - return random word from array of words corresponding to matched key word
  
  
*/

function madlibs(template) {
  let result = template.replace(/adjective/g, replacer);
  result = result.replace(/noun/g, replacer);
  result = result.replace(/adverb/g, replacer);
  result = result.replace(/verb/g, replacer);

  return result;
}

function replacer(match) {
  let numberOfWords = replacementWords[match].length;
  let randomIndex = randomInt(0, numberOfWords - 1);
  return replacementWords[match][randomIndex];
}
...
let replacementWords = {
  adjective: 'quick lazy sleepy noisy hungry'.split(' '),
  noun: 'fox dog head leg tail cat'.split(' '),
  verb: 'jumps lifts bites licks pats'.split(' '),
  adverb: 'easily lazily noisily excitedly'.split(' '),
}
